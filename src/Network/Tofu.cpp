...
std::string sha256FingerprintHex(const std::uint8_t* bytes, std::size_t len) {
    ...
    // sodium_init() is idempotent; the pairing / session paths call it too, but
    // a fingerprint may be computed before any of those on first contact.
