...
#
# Ported from TinkerNorth/satellite packaging/repo/build-apt-repo.sh; keep the
# two in step when the layout changes.
#
# The repo layout we produce (rooted at $PAGES_DIR):
#
#   debian/pool/main/d/dish/dish_<ver>_amd64.deb
...
# downloaded artifacts into. Any *.deb file matching the dish
# naming pattern is copied into the pool.
#
# The .deb targets Debian 13+ (the distro Qt meets the 6.7 floor there);
# docs/PACKAGING.md is the authority on which distro gets which format.
#
...
PAGES_DIR="${1:-}"
STAGING_DIR="${2:-}"
SUITE="${DISH_APT_SUITE:-stable}"
COMPONENT="main"
ARCH="amd64"
ORIGIN="${DISH_APT_ORIGIN:-TinkerNorth}"
LABEL="${DISH_APT_LABEL:-Dish}"
DESCRIPTION="Dish Linux package repository"
...
pool_dir="$PAGES_DIR/debian/pool/${COMPONENT}/d/dish"
mkdir -p "$pool_dir"
shopt -s nullglob
debs=("$STAGING_DIR"/dish_*_${ARCH}.deb)
if [ "${#debs[@]}" -eq 0 ]; then
    echo "::warning::No dish_*_${ARCH}.deb found in $STAGING_DIR; nothing to add"
else
    for deb in "${debs[@]}"; do
