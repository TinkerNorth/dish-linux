...
#
# Ported from TinkerNorth/satellite packaging/repo/build-dnf-repo.sh; keep the
# two in step when the layout changes.
#
# Layout under $PAGES_DIR:
#
#   rpm/x86_64/dish-<ver>-1.x86_64.rpm
#   rpm/x86_64/repodata/
#       ├── repomd.xml
...
#       ├── filelists.xml.gz
#       └── other.xml.gz
#   rpm/dish.repo                ← dnf config users curl into /etc/yum.repos.d/
#
# Usage:
...
shopt -s nullglob
rpms=("$STAGING_DIR"/dish-*."${ARCH}".rpm)
if [ "${#rpms[@]}" -eq 0 ]; then
    echo "::warning::No dish-*.${ARCH}.rpm found in $STAGING_DIR; nothing to add"
else
    for rpm in "${rpms[@]}"; do
...
# ── 4. Drop a dish.repo file users can curl into /etc/yum.repos.d/ ──────────
cat > "$PAGES_DIR/rpm/dish.repo" <<EOF
[dish]
name=Dish (TinkerNorth)
